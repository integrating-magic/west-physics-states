...
(window.loadBubbleMap = function (json) {
  ...
  const uniqueZips = [
    ...new Set(data.map((item) => item.fieldData.PostalCode)),
  ];
  ...
  const groupByZip = uniqueZips.map((code) => {
    ...
    const facility = group
      .map((item) => item.fieldData.FacilityName)
      .join("\n");
  ...
  anychart.onDocumentReady(function () {
    ...
    citiesSeries.labels(true).enabled(true);
    ...
    statesSeries.labels(true).enabled(true);
  ...
  (window.loadBubbleMapWorkLoad = function (json) {
    ...
    const uniqueZips = [
      ...new Set(
        data.map((item) => item.fieldData["Mobile Facilities::PostalCode"])
      ),
    ];
    ...
    const groupByZip = uniqueZips.map((code) => {
      const group = data.filter(
        (item) => item.fieldData["Mobile Facilities::PostalCode"] === code
      );
      const lat = group[0].fieldData["Mobile Facilities::Latitude"];
      const long = group[0].fieldData["Mobile Facilities::Longitude"];
      const uniquePhysicists = [
        ...new Set(group.map((item) => item.fieldData.PhysicistNameList)),
      ];
...
window.saveImage = saveImage("container", "Get Image");